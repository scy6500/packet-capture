...
void got_packet(u_char * args,
    ...
    if (ip -> iph_protocol == IPPROTO_TCP) {
        ...
        int tcp_header_length = tcp -> tcp_offx2 / 4;
        int payload_offset = sizeof(struct ethheader) + ip_header_length + tcp_header_length;
        int packet_length = header -> caplen;
        int payload_length = packet_length - (ip_header_length + tcp_header_length);
